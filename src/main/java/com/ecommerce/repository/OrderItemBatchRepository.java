...
public interface OrderItemBatchRepository extends JpaRepository<OrderItemBatch, Long> {
    ...
    @Query("SELECT oib FROM OrderItemBatch oib WHERE oib.orderItem.shopOrder.order.orderId = :orderId")
    List<OrderItemBatch> findByOrderId(@Param("orderId") Long orderId);

    @Query("SELECT oib FROM OrderItemBatch oib JOIN FETCH oib.warehouse WHERE oib.orderItem.shopOrder.order.orderId = :orderId")
